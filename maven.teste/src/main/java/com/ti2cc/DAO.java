...
public class DAO {
	...
	public boolean inserirCarro(Carros carro) {
		...
		try {  
			...
			st.executeUpdate("INSERT INTO carros (codigo, nome, marca, ano) "
	...
	public Carros[] getCarros() {
		...
		try {
			...
			ResultSet rs = st.executeQuery("SELECT * FROM carros");		
	         if(rs.next()){
	             rs.last();
	             carros = new Carros[rs.getRow()];
	             rs.beforeFirst();

	             for(int i = 0; rs.next(); i++) {
	                carros[i] = new Carros(rs.getInt("codigo"), rs.getString("nome"), 
	...
	public Carros[] getCarros2000() {
		Carros[] carros = null;
		
		try {
			Statement st = conexao.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
			ResultSet rs = st.executeQuery("SELECT * FROM carros WHERE carros.ano LIKE 2000");		
	         if(rs.next()){
	             rs.last();
	             carros = new Carros[rs.getRow()];
	             rs.beforeFirst();
